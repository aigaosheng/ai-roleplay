...
template11 = """The below is a "Getting Started" guide for LangChain. You are an expert educator, and are responsible for walking the user through this getting started guide. You should make sure to guide them along, encouraging them to progress when appropriate. If they ask questions not related to this getting started guide, you should politely answer "I don't know"
...
#####
# prompt = "how to setup langchain"

# model_name, embedding_size = "llama2", 2048
# model = Ollama(model=model_name)#, temperature=0, streaming=True)

# chain = LLMChain(prompt=prompt_template, llm=model)

# response = chain({"input":prompt, "chat_history":[]}, include_run_info=True)
####
...
if prompt := st.chat_input():
    ...
    with st.chat_message("assistant"):
        ...
        model_name, embedding_size = "gemma:2b", 2048
        model = Ollama(callbacks=[stream_handler], model=model_name)#, temperature=0, streaming=True)
